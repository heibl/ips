...
#' 
#' @param phy object of class \code{\link{phylo}}
#' @param exec path to PATHd8 program
#' @param seql sequence length of alignment
#' @param calibration data.frame with 4 columns and as many rows as calibration points. Columns are: taxon 1; taxon 2; one of c("minage", "maxage", "fixage"); age.
#' 
#' @return tree list of ultrametric trees returned from PATHd8 of class \code{\link{phylo}}. First tree is PATHd8 chronogram, which is a calibrated ultrametric tree. Second is a PATH tree, which is a ultrametric tree without calibration.
#' 
#' @references Britton et al (2006). PATHd8---a new method for estimating divergence times in large phylogenetic trees without a molecular clock. Available from the authors (www.math.su.se/PATHd8)
#' @references Britton et al. (2007). Estimating divergence times in large phylogenetic trees. Systematic biology. 56:741--752
#' 
...
pathd8 <- function(phy, exec = "/Applications/PATHd8/PATHd8", seql, calibration){
  ...
  if (!inherits(phy, "phylo"))
    stop("sequences not of class phylo")
  ...
  tree <- write.tree(phy)
  cal <- calibration 
  cal <- paste("mrca: ", cal[,1], ", ", cal[,2], ", ", cal[,3], "=", cal[,4], ";", sep="")
  
  if(!missing(calibration)){
    write(paste("Sequence length= ", 
      seql, ";", 
      " \n \n", 
      tree, 
      "\n \n",
  ...
  tab2 <- plyr::rbind.fill(lapply(strsplit(tab2, " "),function(y) { as.data.frame(t(y), stringsAsFactors=FALSE) }))
