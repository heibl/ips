...
mrbayes.mixed <- function(x, file, nst = 6, 
	...
	# datatype
	# --------
	datatype <- paste("datatype=mixed(DNA:1-", p2-1, 
                    ",Standard:", 		p2, "-", ncha, 
                    ")", sep = "")
	...
	# write MrBayes block 
	bayes <- vector(length = 5)
	bayes[1] <- "begin mrbayes;"
	bayes[2] <- paste("\tlset nst=", nst, " rates=", 		
                    rates, " ngammacat=", ngammacat,";", 
                    sep = "")
	bayes[3] <- paste("\tmcmc nruns=", nruns, " ngen=", 		
                    as.integer(ngen), " printfreq=", 
                    printfreq, 		" samplefreq=", samplefreq, " nchains=", 		nchains, " savebrlens=", savebrlens, 			" temp=", temp, ";", sep = "")
	# added sumt
	bayes[4] <- paste("\tsumt filename=", file, " burnin=", 
